        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 
                  之前的路由跳转，从a路由跳转到b路由，这里在加入购物车进行路由跳转之前，需要发送请求
                  把购买的产品的信息通过请求的方式发送给服务器，服务器进行相应的存储
                -->
...
export default {
  ...
  computed: {
    ...
    // 给子组件的数据
    skuImageList() {
      // 如果服务器的数据没有回来，则skuImageList为一个空数组
  ...
  methods: {
    ...
    changeSkuNum(event) {
      ...
      // 如果用户的输入非法,出现NaN或者小于1
      if (isNaN(value) || value < 1) {
        // 比如输入了字符串或者负数
        this.skuNum = 1;
      } else {
        this.skuNum = parseInt(value); // 解决用户输入的为小数
      }
    },
    // 加入购物车的回调
    async addShopCar() {
      ...
      try {
        ...
        // 路由跳转   在路由跳转的时候还需要将产品的信息带给下一级的路由组件
        // 这种写法也可以，但是地址栏不美观
        // this.$router.push({name:"addcartsuccess",query:{skuInfo:this.skuInfo,skuNum:this.skuNum}});
        // 只带商品的数量(将其余的数据存储在本地中)
        /**
         *  一些简单的数据skuNum，通过query参数的形式带给下一级路由
         *  产品信息的数据【比较复杂】，通过会话存储，不建议用本地存储(不持久化，会话结束后数据消失)
         *  本地存储|会话存储，一般存储的是字符串，不会存对象
         */
