    ...
    <div class="register">
      ...
      <div class="content">
        ...
        <!-- <input
          ...
          v-model="password1"
        /> -->
        <input placeholder="确认密码" v-model="password1" name="password1" 
  ...
  methods: {
    ...
    async getCode() {
      ...
      try {
        ...
        // 将组件中code的值变为仓库中的code值(验证码自己写了,不太正常)
        this.code = this.$store.state.user.code;
      } catch (e) {
        alert(e.message);
      }
    },
    // 用户注册的回调(要保证全部验证成功才能发请求)
    async userRegister() {
      //
      const sucess = await this.$validator.validateAll();
      if (sucess) {
        //  全部表单验证成功再向服务器发请求，进行注册
        try {
