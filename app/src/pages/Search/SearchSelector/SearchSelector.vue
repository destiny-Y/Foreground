  ...
  <div class="clearfix selector">
    <div class="type-wrap logo">
      ...
      <div class="value logos">
        <ul class="logo-list">
          <li v-for="tradmark in trademarkList" :key="tradmark.tmId" @click="tradMarkHandler(tradmark)">
            {{ tradmark.tmName }}
          </li>
...
export default {
  ...
  methods: {
    // “品牌”的事件处理函数
    tradMarkHandler(tradmark) {
      // 点击了某一个“品牌”，还是需要整理参数，向服务器发请求获取数据进行展示
      // 在父组件中发送请求，子组件需要将点击的“品牌”的数据发送给父组件(通过自定义事件来实现)
      this.$emit("tradmarkInfo", tradmark);
    },
    // 点击“品牌属性值”的回调
    attrInfo(attr, attrValue) {
      // 将“属性”数据传递给父组件，并在父组件中发请求
      this.$emit("arrtInfo", attr, attrValue);
    },
  },
};
</script>
