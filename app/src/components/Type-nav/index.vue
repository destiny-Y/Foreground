...
export default {
  ...
  data() {
    return {
      // 存储用户鼠标移上了哪一个分类
      currentIndex: -1, // 表示一开始鼠标不在任何一个分类上
      show: true,
    };
  },
  methods: {
     ...
     *  非正常情况：(用户操作过快)：本身全部的一级分类都应该触发鼠标进入事件，但是经过测试，只有部分触发了
     *  就是由于用户行为过快，导致浏览器反应不过来，如果当前回调函数中有一些大量业务，就有可能出现卡顿现象
     */
    // },
    ...
    goSearch(event) {
      ...
      // 最好的解决方案：利用编程式路由导航+事件委派
      // 利用事件委派存在一些问题：1. 点击的一定是a标签吗？ 2. 路由跳转的时候如何获取到参数？
      /**
       *  事件委派：是把全部的子节点【h3、dt、dl、em】的事件委派给父亲节点
       *  点击a标签的时候，才会进行路由的跳转【怎么能确定点击的一定是a标签?】
       *  存在另一个问题：即使能确定点击的是a标签，如何区分是一级、二级、三级分类的标签
       */
      // 1. 把子节点当中a标签加上自定义属性:data-categoryName，其余的子节点是没有的
      let element = event.target;
      // 获取到当前触发事件的节点【h2、a、dt】，需要带有data-categoryname这样的节点【一定是a标签】
      // 节点有一个属性dataset属性，可以获取节点的自定义属性与属性值
      let { categoryname, category1id, category2id, category3id } =
        element.dataset;
      // console.log("####", element.dataset);
      // 如果标签带有categoryname，一定是a标签
      if (categoryname) {
        // 整理路由跳转的参数
